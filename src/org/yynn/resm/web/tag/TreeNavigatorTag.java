 ...
 * @created 2007-5-20
 */
public class TreeNavigatorTag extends BaseTagSupport {
	...
	protected int handleStartTag() throws JspException {
		...
		WebApplicationContext wactx = RequestContextUtils.getWebApplicationContext(pageContext
				.getRequest());
	 ...
	 * @throws IOException
	 */
	private void init(StringBuffer content, NavigatorTree tree) throws IOException {
		...
		ArrayList<NavigatorTreeNode> nodes = tree.getNodes();
	 ...
	 * @throws IOException
	 */
	private void outTree(StringBuffer content, NavigatorTree tree) throws IOException {
		content.append("<DIV>");
		...
		outBranchTreeNodes(content, tree.getNodes());
	}
	 ...
	 * @throws IOException
	 */
	private void outBranchTreeNodes(StringBuffer content, ArrayList<NavigatorTreeNode> nodes)
			throws IOException {
		...
		for (int i = 0, n = nodes.size(); i < n; i++) {
			...
			content.append("   <TR>");
			if (hasChildren)
				content.append("    <A onClick=\"expandIt('" + nid + "'); return false\" href='#'>");
			...
			content.append("<IMG id=" + nid + "Q src='" + openFolder_file_path
					+ "' width=16 height=22 border=0>");
			content.append("<IMG id=" + nid + "L src='" + openinFolder_file_path
					+ "' width=16 height=22 border=0>");
			content.append("</TD>");
			content.append("    <TD class=color noWrap>" + getMessage(nodes.get(i).getDisplayCode())
					+ "</TD>");
			...
			outLeafTreeNodes(nid, content, nodes.get(i).getChildren(), isEnd);
		}
	 ...
	 * @throws IOException
	 */
	private void outLeafTreeNodes(String parentId, StringBuffer content,
			ArrayList<NavigatorTreeNode> nodes, boolean isEnd) throws IOException {
		if (null == nodes || 0 == nodes.size()) return;
		...
		for (int i = 0, n = nodes.size(); i < n; i++) {
			...
			content.append("</TD>");
			content.append("    <TD noWrap>");
			content.append("    <A class=link href=" + href + " target="
					+ (nodeTarget == null ? target : nodeTarget)
					+ " onMouseOut=MM_swapImgRestore() onMouseOver=MM_swapImage('Image+" + parentId + i
					+ "','','" + nonFolder1_file_path + "',1)>");
			content.append("    <img src=" + nonFolder_file_path + " border=0 name='Image+" + parentId
					+ i + "' onMouseOut=MM_swapImgRestore() onMouseOver=MM_swapImage('Image+" + parentId + i
